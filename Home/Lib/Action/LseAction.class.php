...
class LseAction extends Action
{
    ...
    function _bus($k='110100012424'){
    	$url  = C('LSE')."sisserver.php?query_type=IDQ&data_type=BUSLINE&keywords=$k";
    	$list = get_data_by_get($url);
    ...
    function _pguid($id){
    	...
    	$url  = C('LSE')."sisserver.php?query_type=IDQ&data_type=POI+NEWPOI+BUS&city=total&id=$id";
    	$list = get_data_by_get($url);
    ...
    function boundsSearch($k='酒店',$c=10,$sw,$ne){
    	...
    	$url  = C('LSE')."sisserver.php?query_src=test&user_info=test&query_type=SPQ&geotype=rectangle&sstype=0&geoobj=$b&data_type=POI+NEWPOI&page_num=$c&keywords=".$k;
    	$list = get_data_by_get($url);
    	if ($list['count']!="" && is_array($list['list']) && is_array($list['list']["poi"])) {//返回有效结果
